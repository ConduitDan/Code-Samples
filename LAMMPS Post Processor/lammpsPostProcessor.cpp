...
void LammpsPostProcessor::read_data(int start_ts, int final_ts, int increment) {
    ...
    _trajectoryData.resize((final_ts-start_ts)/increment+1);
    bool success = true;
    ...
    for (int frameNumber = start_ts; frameNumber<=final_ts; frameNumber+=increment){
        ...
        newFrame.readFacet(dihedral_file.c_str()); // if we don't read a facet thats fine
	if (!success) {
		std::cout<<"Failed to read dump "<<frameNumber<<std::endl;
		_trajectoryData.resize((increment-start_ts)/increment);
        return;
    }
        int i = (frameNumber-start_ts)/increment;
        _trajectoryData[i]=(newFrame);
    }
    
}
void LammpsPostProcessor::calculate_area(){
    std::string fileName = _projectFolderPath+"/area.csv";

    FILE* fp;
    char line[200];
    fp = fopen(fileName.c_str(), "w");

    for (auto &frame : _trajectoryData){
        fprintf(fp,"%f,",frame.area());
    }
    fclose(fp);
}
void LammpsPostProcessor::calculate_radius(){
    std::string fileName = _projectFolderPath+"/radius.csv";
    ...
    for (auto &frame : _trajectoryData){
        fprintf(fp,"%f,",frame.radius());
    }
    fclose(fp);
}

void LammpsPostProcessor::calculate_swaps() {
    std::string fileName = _projectFolderPath+"/swaps.csv";

    FILE* fp;
    char line[200];
    fp = fopen(fileName.c_str(), "w");

    for (int i = 0; i<_trajectoryData.size()-1; i++){
        fprintf(fp,"%d,",Geometry::bond_diff(_trajectoryData[i].get_bonds(),_trajectoryData[i+1].get_bonds()));
    }
    fclose(fp);
}

void LammpsPostProcessor::make_movie() {
    POVRayDrawer povMaker;
    for (int i = 0; i<_trajectoryData.size(); i++){
        std::string fileName = _projectFolderPath + "/images/" + std::to_string(i);
        povMaker.write_frame(&_trajectoryData[i],fileName);
    }
}

void LammpsPostProcessor::calcV() {
    double dy = 0.1;
    double minY = 25;
    double maxY = 75;
    int nBins = ceil((maxY-minY)/dy);
    std::cout<<nBins<<" bins\n";
    std::vector<double>velocity(nBins,0.0);
    std::vector<unsigned int>density(nBins,0);
    std::string fileName = _projectFolderPath+"/y_vel.csv";

    std::fstream outfile;
    outfile.open(fileName,std::fstream::out);

    for (auto &frame : _trajectoryData){
        frame.vel_profile(velocity,density,minY,maxY,dy);
    }
    double vel_ave = 0;
    for (int i = 0; i<velocity.size(); i++){
        if (density[i] == 0) vel_ave = 0;
        else vel_ave = velocity[i]/density[i];

        outfile<<minY+dy*i<<','<<vel_ave<<','<<density[i]<<std::endl;
    }
    outfile.close();
...
void LammpsPostProcessor::make_movie_visc() {
    POVRayDrawer povMaker;
    double distance = 500;
    double angle = atan2(50,distance)*2*360/(2*M_PI);
    povMaker.set_viewangle(angle);
    povMaker.set_viewpoint({50,50,distance});
    povMaker.set_look_at({50,50,0});
    for (int i = 0; i<_trajectoryData.size(); i++){
        std::string fileName = _projectFolderPath + "/images/" + std::to_string(i);
        povMaker.write_frame(&_trajectoryData[i],fileName);
    }
...
void LammpsPostProcessor::write_frame_to_input() {
    Frame Baseline = _trajectoryData[_trajectoryData.size()-1];
    std::fstream file;
    file.open("relaxedRefinedShell.ldf", std::fstream::out );
    
    // first write the header
    file<<"# Spherical shell of atoms"<<std::endl;
    file<<"9722 atoms"<<std::endl;
    file<<"29160 bonds"<<std::endl;
    file<<"29160 dihedrals"<<std::endl;
    file<<"1 atom types"<<std::endl;
    file<<"2 bond types"<<std::endl;
    file<<"1 dihedral types"<<std::endl;
    file<<"-50.0 50.0 xlo xhi"<<std::endl;
    file<<"-50.0 50.0 ylo yhi"<<std::endl;
    file<<"-50.0 50.0 zlo zhi"<<std::endl;
    file<<std::endl<<"Atoms"<<std::endl<<std::endl;
    
    int counter = 1;
    for (auto &atom : Baseline.get_atoms()){
        std::array<double,3> position = atom.get_position();
        file<<counter<<" 1 1 "<<position[0]<< " " << position[1] << " " << position[2]<<std::endl;
        counter++;
    } 

    file<<std::endl<<"Bonds"<<std::endl<<std::endl;

    counter = 0;
    for (auto &bond : Baseline.get_bonds()){
        file<<counter<<" "<<bond[2]-1<< " " <<bond[0]+1 << " " <<bond[1]+1<<std::endl;
        counter++;
    }
}

bool LammpsPostProcessor::check_flips() {
    std::string flip_file = _projectFolderPath + "/topology/flips_1.dump";

    return _trajectoryData[0].read_and_check_Flips(flip_file.c_str(),_projectFolderPath);
}

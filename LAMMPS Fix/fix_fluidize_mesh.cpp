...
/* -------------------------------------------------------------------------
   Contributed by Danny Hellstein & Matthew S.E. Peterson @ Brandeis University

   Based on an initial implementation by Cong Qiao @ Brandeis.

   Refactored and  rewritten by Danny Hellstein @ Brandeis University
   Added internal data stuctures to keep track of dihedrals to reduce execution
   time from N^2 -> N

  This fix will make the following dihedral flips using a Metropolis Hasing 
  Algorithm
  ...
  This has the effect of fluidizing otherwise elastic meshes
...
double FixFluidizeMesh::compute_bending_energy(dihedral_type dihedral) {
  ...
  // we use -costheta because lammps defines 180 as flat, the opposite of what we want
